...
class ActiveFire{
	...
	newModel(name, properties){
		//create the new reference for this new model at firebase's top level
		var ref = "/" + name;
		//primitive javascript types (there's also null or undefined, but if you're making a model do u really want the type to have to be null?)
		var primitives = {'string': true, 'number':true, 'boolean': true};
		//set up the json for the properties and dependencies
		//(see end of file for schema.JSON structure manifest)
		var dependencies = {};
		...
		//assume properties is a json object with format
		/* { name: 'string',
					age: 'number',
					kittens: 'kitten'
		} */
		...
		//pull the schema so far from schema.JSON
		jsonfile.readFile('./schema.json', function(err, schema) {

			//if schema.json is empty, this is our first schema, so intialize a new object to add to.
			if(!schema){
				console.log('empty schema. creating new');
				schema = {};
			}
			...
			jsonfile.writeFile('./schema.json', schema, function (err) {
				if(err){
					console.error(err);
				}else{
	...
	create(model, name, properties){

		//read the schema and do error checking on their entry
		jsonfile.readFile('./schema.json', (err, schema) => {
			if (this._checkIfModelExists(schema, model) && this._checkPropTypesAgainstSchema(model, schema, properties)) {
				// Checks if model entry is unique
				this.firebase.database().ref(schema[model]["_ref"]).once('value', (snapshot)=>{
					if (snapshot.child(name).exists()) {
						console.error("Model entry "+ name+" already exists on firebase, if you want to overwrite it, delete it first, then try this command again.")
					}else{

						//write to firebase
						this.firebase.database().ref(schema[model]["_ref"]).child(name).set(properties, function(){
							console.log("Wrote model: "+ model +" entry: "+ name +" to firebase!")
						});
					}
				})
			}
	...
	_checkIfModelExists(schema, model){

		if(!schema){
			console.error('no objects found in schema.json');
			return false;
		}
	...
	_checkPropTypesAgainstSchema(model, schema, properties){

		for (var propertyName in properties) {
			var modelPropertyValue = properties[propertyName];
			//if they added a property that doesnt exist for that model, error out
			if (!schema[model]["properties"].hasOwnProperty(propertyName)){
				...
				if(typeof modelPropertyValue != schema[model]["properties"][propertyName]["type"]){
	...
	//iterate through properties they specified
