...
var ActiveFireCollection = require('./relations');
var ActiveFireObject = require('./afobject');
		...
		class Base {
			...
			find(uniqID){
				return new Promise((resolve, reject) => {
					this.firebase.database().ref('/'+this.modelName+'/'+uniqID).once("value", (snapshot) => {
						resolve(new ActiveFireObject(snapshot.val()));
					})
				})
			}
			...
			findBy(key, value){
				return new Promise((resolve,reject) => {
		    	this.firebase.database().ref('/'+this.modelName).orderByChild(key).equalTo(value).once("value", (snapshot) => {
		    		resolve(new ActiveFireObject(snapshot.val()));
		    	})
		  	})
		  }
		  ...
		  _checkPropTypesAgainstSchema(schema, properties){
		    for (var propertyName in properties) {
		      ...
		      if (!schema.attributes.hasOwnProperty(propertyName) && !schema._relationships.hasOwnProperty(propertyName)){
		        ...
		        //if they entered a property with the wrong type, error out
			      if(!schema._relationships.hasOwnProperty(propertyName) && typeof modelPropertyValue != schema.attributes[propertyName].type){
