...
class ActiveFire{
			...
			if (this._checkIfModelExists(schema.val()) && this._checkPropTypesAgainstSchema(schema.val(), properties)) {
				...
				this.firebase.database().ref(schema.val()._ref).once('value', (snapshot)=>{
					if (snapshot.child(entryName).exists()) {
						console.error("Model entry "+ entryName+" already exists on firebase, if you want to overwrite it, delete it first, then try this command again.")
					}else{
						//write to firebase
						this.firebase.database().ref(schema.val()._ref).child(entryName).set(properties);
						console.log("Wrote modelName: " + modelName + " entry: "+ entryName +" to firebase!")
					}
				})
			}
		})
	...
	_checkIfModelExists(schema){
		if(schema == null){
			console.error("Model not created for this entry type, run activeFire.newModel(modelName, properties) first");
			return false;
		}
		return true;
	}
