	...
	constructor(serviceAccount, databaseURL){
		...
		class Base {
			...
			findBy(key, value){
				...
				return this._queryFirebase().then((res) => {
		  ...
		  _queryFirebase() {
		  	...
		  	return new Promise((resolve,reject) => {
		  		// var returnObject;
		    // 	this.firebase.database().ref('/'+this.modelName).orderByChild(key).equalTo(value).once("value", function(snapshot) {
		    //   returnObject = new ActiveFireCollection(snapshot.val());
		    // 	})
		    	resolve();
		  	})
		  }
